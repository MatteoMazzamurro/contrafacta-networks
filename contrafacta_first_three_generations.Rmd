...
Example of application: compute forward and backward reachability
```{r}
#contrafacta only 
...
###3.6.1 Random selection of date - forward reachability
For each contrafactum, the creation date is selected uniformly at random within the interval provided in the BdT. The temporal network is constructed and the forward reachability of each node is computed. The operation is repeated 100 times.
```{r}
contrafacta_fwd_bkwd_m<-matrix(nrow=312, ncol=200)
all_fwd_bkwd_m<-matrix(nrow=312, ncol=200)
for(m in 1:100){
  #links representing the composition of contrafacta only
{
    #link time options
    #random in interval
   l<-length(contrafacta_and_analogies_with_dates$model_onset)
   contrafacta_link_time_random<-numeric(l)
   for(j in 1:l){
         contrafacta_link_time_random[j]<-apply(data.frame(
       model_onset=runif(1,contrafacta_and_analogies_with_dates$model_onset[j],contrafacta_and_analogies_with_dates$model_terminus[j]),
      contrafactum_onset=ifelse(is.na(contrafacta_and_analogies_with_dates$contrafactum_onset[j]),NA,
        runif(1,contrafacta_and_analogies_with_dates$contrafactum_onset[j],contrafacta_and_analogies_with_dates$contrafactum_terminus[j]))
    ), 1, max)
   }
  
    #default is fixed as beginning of the interval. UPDATE THIS IF REQUIRED
    contrafacta_link_time<-contrafacta_link_time_random
...
#links representing the composition of a song with metrical analogies
{
    #link time options
    #random the interval
   analogies_link_time_random<-numeric(l)
   for(j in 1:l){
         analogies_link_time_random[j]<-apply(data.frame(
       model_onset=runif(1,contrafacta_and_analogies_with_dates$model_onset[j],contrafacta_and_analogies_with_dates$model_terminus[j]),
      contrafactum_onset=ifelse(is.na(contrafacta_and_analogies_with_dates$metrical_affinities_onset[j]),NA,
        runif(1,contrafacta_and_analogies_with_dates$metrical_affinities_onset[j],contrafacta_and_analogies_with_dates$metrical_affinities_terminus[j]))
    ), 1, max)
   }
    ...
    #default is fixed as beginning of the interval. UPDATE THIS IF REQUIRED
    analogies_link_time<-analogies_link_time_random
...
#dynamic
{
    #make temporal network contrafacta
contrafacta_dynamic <- networkDynamic(
    contrafacta_static,
    edge.spells = data.frame("onset"=contrafacta_links$onset,
                             "terminus"=contrafacta_links$terminus,
...
#make temporal network all links 
all_dynamic <- networkDynamic(
        all_static,
        edge.spells = data.frame("onset"=all_links$onset,
                                 "terminus"=all_links$terminus,
...
fwd_reach_contrafacta_only <- tReach(contrafacta_dynamic)
bkwd_reach_contrafacta_only <- tReach(contrafacta_dynamic, direction = "bkwd")
contrafacta_fwd_bkwd_m[,m]<-fwd_reach_contrafacta_only
contrafacta_fwd_bkwd_m[,m+100]<-bkwd_reach_contrafacta_only
#save as dataframe
#contrafacta_fwd_bkwd<-data.frame("fwd"=fwd_reach_contrafacta_only,"bkwd"=bkwd_reach_contrafacta_only,"author_name"=authors_names)
...
fwd_reach_all_contrafacta <- tReach(all_dynamic)
bkwd_reach_all_contrafacta <- tReach(all_dynamic, direction = "bkwd")
all_fwd_bkwd_m[,m]<-fwd_reach_all_contrafacta
all_fwd_bkwd_m[,m+100]<-bkwd_reach_all_contrafacta
#save as dataframe
#all_fwd_bkwd<-data.frame("fwd"=fwd_reach_all_contrafacta,"bkwd"=bkwd_reach_all_contrafacta,"author_name"=authors_names)
}
  print(m)
...
The results from the above computations are analysed
```{r}
#mean coefficient of variation
mean(rowSds(contrafacta_fwd_bkwd_m[,1:100])/mean(contrafacta_fwd_bkwd_m[,1:100]))

#mean quartile coefficient of dispersion
mean((rowQuantiles(all_fwd_bkwd_m[,1:100])[,4]-rowQuantiles(all_fwd_bkwd_m[,1:100])[,2])/(rowQuantiles(all_fwd_bkwd_m[,1:100])[,4]+rowQuantiles(all_fwd_bkwd_m[,1:100])[,2]))

#histogram of quartile coefficient of dispersion
hist((rowQuantiles(all_fwd_bkwd_m[,1:100])[,4]-rowQuantiles(all_fwd_bkwd_m[,1:100])[,2])/(rowQuantiles(all_fwd_bkwd_m[,1:100])[,4]+rowQuantiles(all_fwd_bkwd_m[,1:100])[,2]))
```